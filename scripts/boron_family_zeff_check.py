"""Research check for the boron family B -> Tl.

This additional family test extends the Electron-5 inspired comparison to a
same-valence series with valence 3:
...
FAMILY = [
    ...
    {"symbol": "Ga", "name": "Gallium", "period": 4, "mass_u": 69.723, "radius_pm": 135.0, "ionization_ev": 5.9993},
    {"symbol": "In", "name": "Indium", "period": 5, "mass_u": 114.818, "radius_pm": 156.0, "ionization_ev": 5.7864},
    {"symbol": "Tl", "name": "Thallium", "period": 6, "mass_u": 204.38, "radius_pm": 170.0, "ionization_ev": 6.1082},
]
...
def analyze_family() -> dict:
    ...
    trend_ok = zeff_trend == "increasing"
    ...
    return {
        ...
        "hypothesis": "for the boron family, radius tracks Z_eff better than mass alone",
        "case_control": "B -> Al -> Ga -> In -> Tl same-family series",
        "expected": {
            "radius": "mixed but still ordered overall by the local organizer",
            "ionization": "mixed",
...
def main() -> None:
    ...
    json_path = outdir / f"boron_family_zeff_check_{timestamp}.json"
    txt_path = outdir / f"boron_family_zeff_check_{timestamp}.txt"
    json_payload = {"timestamp": timestamp, **results, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(json_payload, indent=2, ensure_ascii=False), encoding="utf-8")

    with txt_path.open("w", encoding="utf-8") as handle:
        handle.write("Recherche sur la famille du bore B -> Tl\n")
