"""Research check for the tetrel family C -> Pb.

This sixth family test extends the Electron-5 inspired comparison to another
same-valence series:
...
def analyze_family() -> dict:
    ...
    trend_ok = radius_trend == "increasing" and zeff_trend == "increasing"
    ...
    return {
        ...
        "hypothesis": "for the tetrel family, radius tracks Z_eff better than mass alone",
        "case_control": "C -> Si -> Ge -> Sn -> Pb same-family series",
        "expected": {
            "radius": "monotonic increase",
            "ionization": "may be mixed at the heavy end",
            "zeff_proxy": "smooth monotonic increase",
            "mass": "less explanatory than zeff_proxy for the family trend",
        },
        "verdict": verdict,
        "falsifiers": [
            "radius is not monotonic across C -> Pb",
...
def main() -> None:
    ...
    json_path = outdir / f"tetrel_family_zeff_check_{timestamp}.json"
    txt_path = outdir / f"tetrel_family_zeff_check_{timestamp}.txt"
    json_payload = {"timestamp": timestamp, **results, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(json_payload, indent=2, ensure_ascii=False), encoding="utf-8")

    with txt_path.open("w", encoding="utf-8") as handle:
        handle.write("Recherche sur la famille des tetrels C -> Pb\n")
