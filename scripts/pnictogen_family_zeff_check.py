"""Research check for the pnictogen family N -> Bi.

This fifth family test continues the Electron-5 inspired comparison across a
different same-valence series:
...
def analyze_family() -> dict:
    ...
    return {
        ...
        "hypothesis": "for the pnictogen family, radius tracks Z_eff better than mass alone",
        "case_control": "N -> P -> As -> Sb -> Bi same-family series",
        "expected": {
        ...
        "verdict": verdict,
        "falsifiers": [
            "radius is not monotonic across N -> Bi",
            "first ionization energy is not monotonic across N -> Bi",
...
def main() -> None:
    ...
    json_path = outdir / f"pnictogen_family_zeff_check_{timestamp}.json"
    txt_path = outdir / f"pnictogen_family_zeff_check_{timestamp}.txt"
    json_payload = {"timestamp": timestamp, **results, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(json_payload, indent=2, ensure_ascii=False), encoding="utf-8")

    with txt_path.open("w", encoding="utf-8") as handle:
        handle.write("Recherche sur la famille des pnictogenes N -> Bi\n")
