"""Research check for the halogen family F -> At.

This third family test checks whether the Electron-5 derived working rule
still holds outside the alkali families:

- atomic radius should increase down the family,
- first ionization energy should decrease down the family,
- the inferred Z_eff proxy should remain a more useful organizer than mass
  alone for the family trend.
...
FAMILY = [
    {"symbol": "F", "name": "Fluorine", "period": 2, "mass_u": 18.998403163, "radius_pm": 64.0, "ionization_ev": 17.4228},
    {"symbol": "Cl", "name": "Chlorine", "period": 3, "mass_u": 35.45, "radius_pm": 99.0, "ionization_ev": 12.9676},
    {"symbol": "Br", "name": "Bromine", "period": 4, "mass_u": 79.904, "radius_pm": 114.0, "ionization_ev": 11.8138},
    {"symbol": "I", "name": "Iodine", "period": 5, "mass_u": 126.90447, "radius_pm": 133.0, "ionization_ev": 10.4513},
    {"symbol": "At", "name": "Astatine", "period": 6, "mass_u": 210.0, "radius_pm": 150.0, "ionization_ev": 9.5},
]
...
def analyze_family() -> dict:
    ...
    return {
        ...
        "hypothesis": "for the halogen family, radius tracks Z_eff better than mass alone",
        "case_control": "F -> Cl -> Br -> I -> At same-family series",
        "expected": {
        ...
        "verdict": verdict,
        "falsifiers": [
            "radius is not monotonic across F -> At",
            "first ionization energy is not monotonic across F -> At",
...
def main() -> None:
    ...
    json_path = outdir / f"halogen_family_zeff_check_{timestamp}.json"
    txt_path = outdir / f"halogen_family_zeff_check_{timestamp}.txt"
    json_payload = {"timestamp": timestamp, **results, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(json_payload, indent=2, ensure_ascii=False), encoding="utf-8")

    with txt_path.open("w", encoding="utf-8") as handle:
        handle.write("Recherche sur la famille des halogenes F -> At\n")
