"""Research check for the alkaline earth family Be -> Ba.

The goal is to refine the Electron-5 inspired hypothesis by testing another
same-valence family:

- radius should follow the effective charge proxy better than mass alone;
- first ionization energy should decrease smoothly down the family;
- the inferred Z_eff proxy should remain a better organizing variable than
  mass for the family trend.
...
FAMILY = [
    {"symbol": "Be", "name": "Beryllium", "period": 2, "mass_u": 9.0121831, "radius_pm": 112.0, "ionization_ev": 9.3227},
    {"symbol": "Mg", "name": "Magnesium", "period": 3, "mass_u": 24.305, "radius_pm": 160.0, "ionization_ev": 7.6462},
    {"symbol": "Ca", "name": "Calcium", "period": 4, "mass_u": 40.078, "radius_pm": 197.0, "ionization_ev": 6.1132},
    {"symbol": "Sr", "name": "Strontium", "period": 5, "mass_u": 87.62, "radius_pm": 215.0, "ionization_ev": 5.6949},
    {"symbol": "Ba", "name": "Barium", "period": 6, "mass_u": 137.327, "radius_pm": 222.0, "ionization_ev": 5.2117},
]
...
def analyze_family() -> dict:
    ...
    return {
        "family_name": "alkaline_earth_family",
        "family": evaluated,
        "observables": {
        ...
        "hypothesis": "for the alkaline earth family, radius tracks Z_eff better than mass alone",
        "case_control": "Be -> Mg -> Ca -> Sr -> Ba same-valence family",
        "expected": {
        ...
        "verdict": verdict,
        "falsifiers": [
            "radius is not monotonic across Be -> Ba",
            "first ionization energy is not monotonic across Be -> Ba",
...
def main() -> None:
    ...
    json_path = outdir / f"alkaline_earth_family_zeff_check_{timestamp}.json"
    txt_path = outdir / f"alkaline_earth_family_zeff_check_{timestamp}.txt"
    json_payload = {"timestamp": timestamp, **results, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(json_payload, indent=2, ensure_ascii=False), encoding="utf-8")

    with txt_path.open("w", encoding="utf-8") as handle:
        handle.write("Recherche sur la famille alcalino-terreuse Be -> Ba\n")
