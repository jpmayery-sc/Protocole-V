...
from runv8atomic_suite import run_suite


def test_v8atomic_suite_is_supported(tmp_path):
    result = run_suite(tmp_path)

    assert result["suite"] == "v8atomicsuite"
    assert result["total"] == 3
    assert result["supported_count"] == 3
    assert result["overall_verdict"] == "supported"

    labels = {item["label"] for item in result["items"]}
    assert labels == {"v8_rayon", "v8_level", "v8_stability"}
    assert Path(result["json_path"]).exists()
    assert Path(result["txt_path"]).exists()


def test_v8atomic_suite_defaults_to_workspace_results_dir():
