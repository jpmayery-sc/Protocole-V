"""Run the V9 atomic suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v9crit_check", "v9_crit"),
        ("v9fine_check", "v9_fine"),
        ("v9atommap_check", "v9_map"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v9atomicsuite_summary_{timestamp}.json"
    txt_path = result_dir / f"v9atomicsuite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V9 atomic suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v9crit_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v9fine_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v9atommap_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V9 atomic suite and summarize it.")
