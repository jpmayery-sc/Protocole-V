...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v45beta_functions_check", "v45_beta_functions"),
        ("v45rg_flow_check", "v45_rg_flow"),
        ("v45fixed_points_check", "v45_fixed_points"),
        ("v45stability_check", "v45_stability"),
        ("v45synthesis_check", "v45_synthesis"),
    ]
    ...
    return {
        "suite": "v45rgflow_suite",
        "timestamp": time.strftime("%Y%m%d-%H%M%SZ"),
        "v45_global_verdict": global_verdict,
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v45rgflow_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v45rgflow_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=True), encoding="utf-8")

    lines = [
        "V45 RG flow suite summary",
        f"timestamp: {timestamp}",
        f"v45_global_verdict: {summary['v45_global_verdict']}",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v45_rgflow"

    commands = [
        [sys.executable, str(scripts_dir / "v45beta_functions_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v45rg_flow_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v45fixed_points_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v45stability_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v45synthesis_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V45 RG flow suite and summarize it.")
