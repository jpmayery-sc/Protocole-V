"""Run the fine-variation hard checks and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("fine_variation_hard_check", "pnictogen_hard"),
        ("fine_variation_lanthanide_hard_check", "lanthanide_hard"),
        ("fine_variation_boron_hard_check", "boron_hard"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"fine_variation_hard_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"fine_variation_hard_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "Fine variation hard suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    result_dir = Path(output_dir) if output_dir is not None else root / "python" / "results" / "fine_variation_hard"

    commands = [
        [sys.executable, str(scripts_dir / "fine_variation_hard_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "fine_variation_lanthanide_hard_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "fine_variation_boron_hard_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the fine-variation hard checks and summarize them.")
