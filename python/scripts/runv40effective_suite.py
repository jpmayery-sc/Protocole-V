...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v40lagrangian_check", "v40_lagrangian"),
        ("v40projection_check", "v40_projection"),
        ("v40stability_check", "v40_stability"),
        ("v40synthesis_check", "v40_synthesis"),
    ]
    ...
    return {
        "suite": "v40effective_suite",
        "timestamp": time.strftime("%Y%m%d-%H%M%SZ"),
        "v40_global_verdict": global_verdict,
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v40effective_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v40effective_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V40 effective suite summary",
        f"timestamp: {timestamp}",
        f"v40_global_verdict: {summary['v40_global_verdict']}",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v40_effective_model"

    commands = [
        [sys.executable, str(scripts_dir / "v40lagrangian_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v40projection_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v40stability_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v40synthesis_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V40 effective suite and summarize it.")
