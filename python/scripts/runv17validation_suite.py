"""Run the V17 validation suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v17reference_check", "v17_reference"),
        ("v17compare_check", "v17_compare"),
        ("v17deviation_check", "v17_deviation"),
        ("v17verdict_check", "v17_verdict"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v17validation_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v17validation_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V17 validation suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v17reference_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v17compare_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v17deviation_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v17verdict_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V17 validation suite and summarize it.")
