"""Run the V20 research suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v20geo_check", "v20_geo"),
        ("v20torsion_check", "v20_torsion"),
        ("v20hierarchy_check", "v20_hierarchy"),
        ("v20verdict_check", "v20_verdict"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v20research_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v20research_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V20 research suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v20geo_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v20torsion_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v20hierarchy_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v20verdict_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V20 research suite and summarize it.")
