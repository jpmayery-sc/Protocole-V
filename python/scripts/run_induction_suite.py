"""Run the induction checks and summarize the results."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("electromagnetic_induction_check", "weak_induction"),
        ("strong_induction_check", "strong_induction"),
    ]
    ...
    supported = sum(1 for item in items if item["verdict"] in {"supported", "conforme strict"})
    overall_verdict = "supported" if supported == len(items) else "contradicted"
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"induction_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"induction_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "Induction suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    python_scripts_dir = root / "python" / "scripts"
    result_dir = Path(output_dir) if output_dir is not None else root / "results" / "result-analyse"

    commands = [
        [sys.executable, str(scripts_dir / "electromagnetic_induction_check.py")],
        [sys.executable, str(python_scripts_dir / "strong_induction_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the induction checks and summarize them.")
