...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v34muon_check", "v34_muon"),
        ("v34neutrinos_check", "v34_neutrinos"),
        ("v34tester_summary_check", "v34_tester_summary"),
    ]
    ...
    rejected_reasons = []
    parameter_ranges = {}
    for stem, _label in report_specs:
        report_path = latest_report(result_dir, stem)
        report = json.loads(report_path.read_text(encoding="utf-8"))
        rejected_reasons.extend(report.get("rejected_reasons", []))
        parameter_ranges[stem] = report.get("parameter_ranges", {})

    return {
        "suite": "v34geometricfalsification_suite",
        "timestamp": time.strftime("%Y%m%d-%H%M%SZ"),
        "global_verdict": overall_verdict,
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v34geometricfalsification_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v34geometricfalsification_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V34 geometric falsification suite summary",
        f"timestamp: {timestamp}",
        f"global_verdict: {summary['global_verdict']}",
        f"supported_count: {summary['supported_count']}/{summary['total']}",
        f"rejected_reasons_count: {len(summary['rejected_reasons'])}",
        "",
        "Items:",
    ]
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v34_geometric_falsification"

    commands = [
        [sys.executable, str(scripts_dir / "v34muon_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v34neutrinos_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v34tester_summary_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V34 geometric falsification suite and summarize it.")
