...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v27qgr_check", "v27_qgr"),
        ("v27renormalisation_check", "v27_renormalisation"),
        ("v27multiscale_check", "v27_multiscale"),
        ("v27observablesadvanced_check", "v27_observables_advanced"),
        ("v27publicationadvanced_check", "v27_publication_advanced"),
        ("v27verdict_check", "v27_verdict"),
    ]
    ...
    for stem, label in report_specs:
        ...
        verdict = report.get("verdict") or report.get("physics_verdict") or report.get("v27_verdict") or "unknown"
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v27qgr_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v27qgr_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V27 QGR suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v27qgr_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v27renormalisation_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v27multiscale_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v27observablesadvanced_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v27publicationadvanced_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v27verdict_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V27 QGR suite and summarize it.")
