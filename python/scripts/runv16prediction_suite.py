"""Run the V16 prediction suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v16internal_check", "v16_internal"),
        ("v16external_check", "v16_external"),
        ("v16check_check", "v16_check"),
        ("v16export_check", "v16_export"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v16prediction_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v16prediction_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V16 prediction suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v16internal_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v16external_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v16check_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v16export_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V16 prediction suite and summarize it.")
