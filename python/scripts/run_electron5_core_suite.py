"""Run the Electron-5 core checks and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("quark_mass_derivation_check", "quark_mass_derivation"),
        ("hadron_collective_mode_check", "hadron_collective_mode"),
        ("pmns_neutrino_check", "pmns_neutrino"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"electron5_core_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"electron5_core_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "Electron-5 core suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    result_dir = Path(output_dir) if output_dir is not None else root / "python" / "results" / "electron5_core"

    commands = [
        [sys.executable, str(scripts_dir / "quark_mass_derivation_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "hadron_collective_mode_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "pmns_neutrino_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the Electron-5 core checks and summarize them.")
