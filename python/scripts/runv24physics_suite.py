"""Run the V24 physics suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v24axioms_check", "v24_axioms"),
        ("v24lagrangian_check", "v24_lagrangian"),
        ("v24effectivephysics_check", "v24_effectivephysics"),
        ("v24physicstests_check", "v24_physicstests"),
        ("v24verdict_check", "v24_verdict"),
    ]
    ...
    for stem, label in report_specs:
        ...
        verdict = report.get("verdict") or report.get("physics_verdict") or report.get("v24_verdict") or "unknown"
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v24physics_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v24physics_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V24 physics suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v24axioms_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v24lagrangian_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v24effectivephysics_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v24physicstests_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v24verdict_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V24 physics suite and summarize it.")
