...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v36scales_check", "v36_scales"),
        ("v36strong_check", "v36_strong"),
        ("v36tester_summary_check", "v36_tester_summary"),
    ]
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v36scalestrong_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v36scalestrong_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V36 scale strong suite summary",
        f"timestamp: {timestamp}",
        f"v36_global_verdict: {summary['v36_global_verdict']}",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v36_scale_strong"

    commands = [
        [sys.executable, str(scripts_dir / "v36scales_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v36strong_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v36tester_summary_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V36 scale/strong suite and summarize it.")
