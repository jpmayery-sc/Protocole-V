...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v31interpretation_check", "v31_interpretation"),
        ("v31synthesis_check", "v31_synthesis"),
        ("v31tester_summary_check", "v31_tester_summary"),
    ]
    ...
    return {
        "suite": "v31geometricinterpretation_suite",
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v31geometricinterpretation_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v31geometricinterpretation_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V31 geometric interpretation suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v31_geometric_interpretation"

    commands = [
        [sys.executable, str(scripts_dir / "v31interpretation_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v31synthesis_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v31tester_summary_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V31 geometric interpretation suite and summarize it.")
