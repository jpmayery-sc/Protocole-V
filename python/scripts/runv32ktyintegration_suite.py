...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v32math_check", "v32_math"),
        ("v32predictions_check", "v32_predictions"),
        ("v32tester_summary_check", "v32_tester_summary"),
    ]
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v32ktyintegration_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v32ktyintegration_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V32 K/T/Y integration suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v32_kty_integration"

    commands = [
        [sys.executable, str(scripts_dir / "v32math_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v32predictions_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v32tester_summary_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V32 K/T/Y integration suite and summarize it.")
