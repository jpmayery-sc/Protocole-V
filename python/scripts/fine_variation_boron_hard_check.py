"""Hard validation of the fine-variation equivalence on the boron family.

The boron family is more irregular than alkalis and halogens, but less extreme
than lanthanides. This script tests whether a geometric fine correction and an
energetic fine correction remain locally equivalent on:
- the full B -> Tl family,
- the repaired core B -> Al -> In,
- the edge-trimmed view B -> Al -> In.
"""
...
FULL_FAMILY = [
    {"name": "B", "period": 2, "radius_pm": 87.0, "ionization_ev": 8.298},
    {"name": "Al", "period": 3, "radius_pm": 143.0, "ionization_ev": 5.9858},
    {"name": "Ga", "period": 4, "radius_pm": 135.0, "ionization_ev": 5.9993},
    {"name": "In", "period": 5, "radius_pm": 156.0, "ionization_ev": 5.7864},
    {"name": "Tl", "period": 6, "radius_pm": 170.0, "ionization_ev": 6.1082},
]

REPAIRED_CORE = [FULL_FAMILY[0], FULL_FAMILY[1], FULL_FAMILY[3]]
EDGE_TRIMMED = [FULL_FAMILY[0], FULL_FAMILY[1], FULL_FAMILY[2], FULL_FAMILY[3]]
...
def run_check(output_dir: str | Path | None = None) -> dict:
    ...
    families = {
        "boron_full": evaluate_family(FULL_FAMILY),
        "boron_repaired_core": evaluate_family(REPAIRED_CORE),
        "boron_edge_trimmed": evaluate_family(EDGE_TRIMMED),
    }
    ...
    json_path = outdir / f"fine_variation_boron_hard_check_{timestamp}.json"
    txt_path = outdir / f"fine_variation_boron_hard_check_{timestamp}.txt"
    json_path.write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "Fine variation boron hard check",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Hard validation of the fine-variation equivalence on the boron family.")
