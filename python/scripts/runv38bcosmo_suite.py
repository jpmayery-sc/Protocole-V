...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v38bhubble_check", "v38b_hubble"),
        ("v38blambda_check", "v38b_lambda"),
        ("v38bgrowth_check", "v38b_growth"),
        ("v38btester_summary_check", "v38b_tester_summary"),
    ]
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v38bcosmo_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v38bcosmo_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V38b cosmology suite summary",
        f"timestamp: {timestamp}",
        f"v38b_global_verdict: {summary['v38b_global_verdict']}",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v38b_cosmology"

    commands = [
        [sys.executable, str(scripts_dir / "v38bhubble_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v38blambda_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v38bgrowth_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v38btester_summary_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V38b cosmology suite and summarize it.")
