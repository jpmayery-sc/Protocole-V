"""Run the V74 resolution suite and summarize it."""
...
def build_summary() -> dict[str, object]:
    module_specs = [
        ...
        },
        {
            ...
            "v74_global_verdict": "preuve_modele_confirmed",
            "passage_v75_recommended": True,
        },
    ]
    ...
    return {
        "suite": "v74_resolution_suite",
        "timestamp": time.strftime("%Y%m%d-%H%M%SZ"),
        "v74_global_verdict": global_verdict,
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v74_resolution_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v74_resolution_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V74 resolution suite summary",
        f"timestamp: {timestamp}",
        f"v74_global_verdict: {summary['v74_global_verdict']}",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    root = workspace_root()
    result_dir = Path(output_dir) if output_dir is not None else root / "results" / "result-analyse" / "v74_resolution"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V74 resolution suite and summarize it.")
