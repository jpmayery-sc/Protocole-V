...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v33math_check", "v33_math"),
        ("v33predictions_check", "v33_predictions"),
        ("v33tester_summary_check", "v33_tester_summary"),
    ]
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v33ktyrefinement_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v33ktyrefinement_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V33 K/T/Y refinement suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v33_kty_refinement"

    commands = [
        [sys.executable, str(scripts_dir / "v33math_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v33predictions_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v33tester_summary_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V33 K/T/Y refinement suite and summarize it.")
