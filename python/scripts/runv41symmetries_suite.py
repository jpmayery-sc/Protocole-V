...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v41setup_check", "v41_setup"),
        ("v41continuous_check", "v41_continuous"),
        ("v41discrete_check", "v41_discrete"),
        ("v41geometric_check", "v41_geometric"),
        ("v41conserved_check", "v41_conserved"),
        ("v41synthesis_check", "v41_synthesis"),
    ]
    ...
    return {
        "suite": "v41symmetries_suite",
        "timestamp": time.strftime("%Y%m%d-%H%M%SZ"),
        "v41_global_verdict": global_verdict,
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v41symmetries_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v41symmetries_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=True), encoding="utf-8")

    lines = [
        "V41 symmetries suite summary",
        f"timestamp: {timestamp}",
        f"v41_global_verdict: {summary['v41_global_verdict']}",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v41_symmetry_analysis"

    commands = [
        [sys.executable, str(scripts_dir / "v41setup_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v41continuous_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v41discrete_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v41geometric_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v41conserved_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v41synthesis_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V41 symmetry suite and summarize it.")
