"""Run the V76 lithium diagnostics around the V80B fixed point."""
...
def plot_scan(rows: list[dict[str, float | bool]], output_dir: Path) -> list[str]:
    ...
    for key, obs, sigma, title, ylabel in series:
        ...
        plt.plot(deltas, values, color="#1f4e79", linewidth=2, label="V76 proxy")
...
def summarize(rows: list[dict[str, float | bool]], plot_paths: list[str], csv_path: Path, output_dir: Path) -> dict[str, object]:
    ...
    if bool(center["passes_all_direct"]) and bool(center["passes_all_total"]) and rate_passes_total == rate_total:
        verdict = "resolution_robuste"
        verdict_text = "Li résolu de façon robuste"
    elif bool(center["passes_all_direct"]):
        verdict = "resolution_fine"
        verdict_text = "Li résolu de façon fine / tuning"
    else:
        verdict = "tension_persistante"
        verdict_text = "Li non résolu"

    summary = {
        "suite": "v76_li_diagnostics",
...
def write_summary(summary: dict[str, object], output_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = output_dir / f"v76_li_diagnostics_summary_{timestamp}.json"
    txt_path = output_dir / f"v76_li_diagnostics_summary_{timestamp}.txt"
    ...
    lines = [
        "V76 lithium diagnostics summary",
...
def run_diagnostics(
    ...
    root = workspace_root()
    result_dir = Path(output_dir) if output_dir is not None else root / "results" / "result-analyse" / "v76_li_diagnostics"

    rows = scan_window(delta_min, delta_max, step)
    csv_path = result_dir / "v76_li_diagnostics_scan.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V76 lithium diagnostics around epsilon_best.")
    ...
    result = run_diagnostics(args.delta_min, args.delta_max, args.step, args.output_dir)
