...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v39paramspace_check", "v39_paramspace"),
        ("v39likelihoods_check", "v39_likelihoods"),
        ("v39mcmc_check", "v39_mcmc"),
        ("v39diagnostics_check", "v39_diagnostics"),
        ("v39synthesis_check", "v39_synthesis"),
    ]
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v39mcmc_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v39mcmc_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V39 mcmc suite summary",
        f"timestamp: {timestamp}",
        f"v39_global_verdict: {summary['v39_global_verdict']}",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v39_mcmc"

    commands = [
        [sys.executable, str(scripts_dir / "v39paramspace_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v39likelihoods_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v39mcmc_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v39diagnostics_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v39synthesis_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V39 MCMC suite and summarize it.")
