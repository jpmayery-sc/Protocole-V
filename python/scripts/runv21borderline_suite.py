"""Run the V21 borderline-resolution suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v21borderline_check", "v21_borderline"),
        ("v21recovery_check", "v21_recovery"),
        ("v21resolution_check", "v21_resolution"),
        ("v21verdict_check", "v21_verdict"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v21borderline_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v21borderline_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V21 borderline suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v21borderline_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v21recovery_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v21resolution_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v21verdict_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V21 borderline-resolution suite and summarize it.")
