...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v46tau_gminus2_check", "v46_tau_gminus2"),
        ("v46tau_decays_check", "v46_tau_decays"),
        ("v46tau_lfu_check", "v46_tau_lfu"),
        ("v46tau_stability_check", "v46_tau_stability"),
        ("v46synthesis_check", "v46_synthesis"),
    ]
    ...
    return {
        ...
        "v46_global_verdict": global_verdict,
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v46tau_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v46tau_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=True), encoding="utf-8")

    lines = [
        "V46 tau suite summary",
        f"timestamp: {timestamp}",
        f"v46_global_verdict: {summary['v46_global_verdict']}",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v46_tau"

    commands = [
        [sys.executable, str(scripts_dir / "v46tau_gminus2_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v46tau_decays_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v46tau_lfu_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v46tau_stability_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v46synthesis_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V46 tau suite and summarize it.")
