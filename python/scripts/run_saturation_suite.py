"""Run the saturation magnetic check and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_path = latest_report(result_dir, "strong_nonlinear_magnetic_response_check")
    report = json.loads(report_path.read_text(encoding="utf-8"))
    verdict = report.get("overall_verdict") or report.get("verdict") or "unknown"

    return {
        "suite": "saturation_magnetic",
        ...
        "items": [
            {
                "label": "strong_nonlinear_magnetic_response",
                "verdict": verdict,
                "json_path": str(report_path),
                "txt_path": report.get("txt_path") or str(report_path.with_suffix(".txt")),
                "hypothesis": "the magnetic response saturates at strong drive while remaining linear at low field",
            }
        ],
    }
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"saturation_magnetic_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"saturation_magnetic_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "Saturation magnetic suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    command = [sys.executable, str(scripts_dir / "strong_nonlinear_magnetic_response_check.py")]
    run_script(command)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the saturation magnetic check and summarize it.")
