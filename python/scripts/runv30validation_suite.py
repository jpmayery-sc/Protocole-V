...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v30anomalies_check", "v30_anomalies"),
        ("v30validation_check", "v30_validation"),
        ("v30table_check", "v30_table"),
        ("v30tester_summary_check", "v30_tester_summary"),
    ]
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v30validation_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v30validation_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V30 validation suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v30_validation"

    commands = [
        [sys.executable, str(scripts_dir / "v30anomalies_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v30validation_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v30table_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v30tester_summary_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V30 validation suite and summarize it.")
