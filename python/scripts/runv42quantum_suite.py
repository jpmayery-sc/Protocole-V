...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v42fields_check", "v42_fields"),
        ("v42propagators_check", "v42_propagators"),
        ("v42interactions_check", "v42_interactions"),
        ("v42loops_check", "v42_loops"),
        ("v42renormality_check", "v42_renormality"),
        ("v42synthesis_check", "v42_synthesis"),
    ]
    ...
    return {
        "suite": "v42quantum_suite",
        "timestamp": time.strftime("%Y%m%d-%H%M%SZ"),
        "v42_global_verdict": global_verdict,
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v42quantum_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v42quantum_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=True), encoding="utf-8")

    lines = [
        "V42 quantum suite summary",
        f"timestamp: {timestamp}",
        f"v42_global_verdict: {summary['v42_global_verdict']}",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v42_quantum_structure"

    commands = [
        [sys.executable, str(scripts_dir / "v42fields_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v42propagators_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v42interactions_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v42loops_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v42renormality_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v42synthesis_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V42 quantum suite and summarize it.")
