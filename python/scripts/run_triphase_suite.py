"""Run the three-phase current check and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_path = latest_report(result_dir, "three_phase_current_check")
    ...
    return {
        ...
        "items": [
            {
                "label": "three_phase_current",
                "verdict": verdict,
                "json_path": str(report_path),
                "txt_path": report.get("report_path") or str(report_path.with_suffix(".txt")),
                "hypothesis": "the balanced three-phase system cancels the neutral current and loses symmetry when perturbed",
            }
        ],
    }
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"triphase_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"triphase_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "Triphase suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    result_dir = Path(output_dir) if output_dir is not None else root / "python" / "results" / "triphase"

    command = [sys.executable, str(scripts_dir / "three_phase_current_check.py"), "--output-dir", str(result_dir)]
    run_script(command)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the triphase check and summarize it.")
