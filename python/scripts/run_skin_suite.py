"""Run the skin-effect check and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_path = latest_report(result_dir, "skin_effect_check")
        ...
        "items": [
            {
                ...
                "hypothesis": "the skin depth decreases with frequency and responds correctly to resistivity and permeability",
            }
        ],
    }
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"skin_effect_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"skin_effect_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "Skin effect suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    command = [sys.executable, str(scripts_dir / "skin_effect_check.py")]
    run_script(command)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the skin-effect check and summarize it.")
