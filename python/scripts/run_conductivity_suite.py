"""Run the material conductivity check and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_path = latest_report(result_dir, "material_conductivity_check")
    report = json.loads(report_path.read_text(encoding="utf-8"))
    verdict = report.get("verdict") or report.get("overall_verdict") or "unknown"

    return {
        "suite": "material_conductivity",
        ...
        "items": [
            {
                "label": "material_conductivity",
                "verdict": verdict,
                "json_path": str(report_path),
                "txt_path": report.get("report_path") or str(report_path.with_suffix(".txt")),
                "hypothesis": "copper, iron, aluminum, and silicon occupy distinct conductivity regimes under the expected perturbations",
            }
        ],
    }
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"material_conductivity_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"material_conductivity_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "Material conductivity suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    command = [sys.executable, str(scripts_dir / "material_conductivity_check.py"), "--output-dir", str(result_dir)]
    run_script(command)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the material conductivity check and summarize it.")
