"""Run the V6 D2/electron suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("d2motionfromalphaport_check", "d2_motion_from_alpha_port"),
        ("electronchannelfromalphacheck", "electron_channel_from_alpha"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v6_d2_electron_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v6_d2_electron_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V6 D2/electron suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "d2motionfromalphaport_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "electronchannelfromalphacheck.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V6 D2/electron suite and summarize it.")
