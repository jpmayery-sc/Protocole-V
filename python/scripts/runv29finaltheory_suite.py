...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v29theoryfinal_check", "v29_theory_final"),
        ("v29articlelatexfull_check", "v29_article_latex_full"),
        ("v29hpcsimulation_check", "v29_hpc_simulation"),
        ("v29observationaltests_check", "v29_observational_tests"),
        ("v29verdict_check", "v29_verdict"),
    ]
    ...
    for stem, label in report_specs:
        ...
        verdict = report.get("verdict") or report.get("physics_verdict") or report.get("v29_verdict") or "unknown"
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v29finaltheory_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v29finaltheory_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V29 final theory suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v29theoryfinal_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v29articlelatexfull_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v29hpcsimulation_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v29observationaltests_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v29verdict_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V29 final theory suite and summarize it.")
