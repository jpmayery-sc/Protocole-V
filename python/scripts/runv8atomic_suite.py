"""Run the V8 atomic suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v8rayon_check", "v8_rayon"),
        ("v8level_check", "v8_level"),
        ("v8stability_check", "v8_stability"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v8atomicsuite_summary_{timestamp}.json"
    txt_path = result_dir / f"v8atomicsuite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V8 atomic suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v8rayon_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v8level_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v8stability_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V8 atomic suite and summarize it.")
