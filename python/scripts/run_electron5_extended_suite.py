"""Run the Electron-5 extended checks and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("quartz_d2_coherence_check", "quartz_d2_coherence"),
        ("quartz_gluon_zeff_separation_check", "quartz_gluon_zeff_separation"),
        ("fine_variation_equivalence_check", "fine_variation_equivalence"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"electron5_extended_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"electron5_extended_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "Electron-5 extended suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    result_dir = Path(output_dir) if output_dir is not None else root / "python" / "results" / "electron5_extended"

    commands = [
        [sys.executable, str(scripts_dir / "quartz_d2_coherence_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "quartz_gluon_zeff_separation_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "fine_variation_equivalence_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the Electron-5 extended checks and summarize them.")
