...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v35muon_scan_check", "v35_muon"),
        ("v35neutrino_scan_check", "v35_neutrinos"),
        ("v35tester_summary_check", "v35_tester_summary"),
    ]
    ...
    return {
        "suite": "v35numericcalibration_suite",
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v35numericcalibration_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v35numericcalibration_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V35 numeric calibration suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v35_numeric_calibration"

    commands = [
        [sys.executable, str(scripts_dir / "v35muon_scan_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v35neutrino_scan_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v35tester_summary_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V35 numeric calibration suite and summarize it.")
