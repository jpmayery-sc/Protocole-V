...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v48mass_model_check", "v48_mass_model"),
        ("v48mixing_check", "v48_mixing"),
        ("v48sterile_dynamics_check", "v48_sterile_dynamics"),
        ("v48cosmo_constraints_check", "v48_cosmo_constraints"),
        ("v48decays_check", "v48_decays"),
        ("v48synthesis_check", "v48_synthesis"),
    ]
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v48neutrinos_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v48neutrinos_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=True), encoding="utf-8")

    lines = [
        "V48 neutrinos suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v48_neutrinos"

    commands = [
        [sys.executable, str(scripts_dir / "v48mass_model_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v48mixing_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v48sterile_dynamics_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v48cosmo_constraints_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v48decays_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v48synthesis_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V48 neutrinos suite and summarize it.")
