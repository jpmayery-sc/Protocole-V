...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v26qft_check", "v26_qft"),
        ("v26perturbations_check", "v26_perturbations"),
        ("v26simulation_check", "v26_simulation"),
        ("v26observables_check", "v26_observables"),
        ("v26publication_check", "v26_publication"),
        ("v26verdict_check", "v26_verdict"),
    ]
    ...
    for stem, label in report_specs:
        ...
        verdict = report.get("verdict") or report.get("physics_verdict") or report.get("v26_verdict") or "unknown"
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v26quantumgeometry_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v26quantumgeometry_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V26 quantum geometry suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v26qft_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v26perturbations_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v26simulation_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v26observables_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v26publication_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v26verdict_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V26 quantum geometry suite and summarize it.")
