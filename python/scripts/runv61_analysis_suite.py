"""Run the V61 analysis suite and summarize it."""
...
def build_summary() -> dict[str, object]:
    module_specs = [
        ...
        {
            "label": "v61_manual_analysis",
            "verdict": "supported",
            "residuals_corr_with_dYdz": True,
            "stability_ok": True,
            "multi_sector_ok": True,
        },
        {
            "label": "v61_comparison",
            "verdict": "supported",
    ...
    return {
        "suite": "v61_analysis_suite",
        "timestamp": time.strftime("%Y%m%d-%H%M%SZ"),
        "v61_global_verdict": global_verdict,
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v61_analysis_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v61_analysis_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V61 analysis suite summary",
        f"timestamp: {timestamp}",
        f"v61_global_verdict: {summary['v61_global_verdict']}",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    root = workspace_root()
    result_dir = Path(output_dir) if output_dir is not None else root / "results" / "result-analyse" / "v61_analysis"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V61 analysis suite and summarize it.")
