...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v43colliders_check", "v43_colliders"),
        ("v43flavour_check", "v43_flavour"),
        ("v43cosmology_check", "v43_cosmology"),
        ("v43astro_check", "v43_astro"),
        ("v43synthesis_check", "v43_synthesis"),
    ]
    ...
    return {
        ...
        "v43_global_verdict": global_verdict,
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v43predictions_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v43predictions_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=True), encoding="utf-8")

    lines = [
        "V43 predictions suite summary",
        f"timestamp: {timestamp}",
        f"v43_global_verdict: {summary['v43_global_verdict']}",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v43_predictions"

    commands = [
        [sys.executable, str(scripts_dir / "v43colliders_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v43flavour_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v43cosmology_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v43astro_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v43synthesis_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V43 prediction suite and summarize it.")
