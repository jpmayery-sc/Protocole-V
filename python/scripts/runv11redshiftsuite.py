"""Run the V11 redshift suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v11geo_check", "v11_geo"),
        ("v11atom_check", "v11_atom"),
        ("v11canal_check", "v11_canal"),
        ("v11total_check", "v11_total"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v11redshift_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v11redshift_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V11 redshift suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v11geo_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v11atom_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v11canal_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v11total_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V11 redshift suite and summarize it.")
