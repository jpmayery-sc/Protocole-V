"""Run the alpha and orbital checks and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("alpha_constant_check", "alpha_constant"),
        ("alpha_phase_observable_check", "alpha_phase_observable"),
        ("shell_subshell_check", "shell_subshell"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"alpha_orbitals_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"alpha_orbitals_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "Alpha / orbitals suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    result_dir = Path(output_dir) if output_dir is not None else root / "python" / "results" / "alpha_orbitals"

    commands = [
        [sys.executable, str(scripts_dir / "alpha_constant_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "alpha_phase_observable_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "shell_subshell_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the alpha and orbital checks and summarize them.")
