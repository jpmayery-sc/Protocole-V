"""Run the V15 consolidation suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v15stability_check", "v15_stability"),
        ("v15robustness_check", "v15_robustness"),
        ("v15lock_check", "v15_lock"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v15consolidation_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v15consolidation_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V15 consolidation suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v15stability_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v15robustness_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v15lock_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V15 consolidation suite and summarize it.")
