"""Run the V23 theory synthesis suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v23schema_check", "v23_schema"),
        ("v23invariants_check", "v23_invariants"),
        ("v23domain_check", "v23_domain"),
        ("v23openquestions_check", "v23_openquestions"),
        ("v23verdict_check", "v23_verdict"),
    ]

    supported_values = {"supported", "supported-partial", "coherent_model"}
    items = []
    supported = 0
    for stem, label in report_specs:
        report_path = latest_report(result_dir, stem)
        report = json.loads(report_path.read_text(encoding="utf-8"))
        verdict = report.get("verdict") or report.get("v23verdict") or "unknown"
        txt_path = report.get("txt_path") or str(report_path.with_suffix(".txt"))
        if verdict in supported_values:
            supported += 1
        items.append(
            {
                "label": label,
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v23theorysynthesis_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v23theorysynthesis_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V23 theory synthesis suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v23schema_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v23invariants_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v23domain_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v23openquestions_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v23verdict_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V23 theory synthesis suite and summarize it.")
