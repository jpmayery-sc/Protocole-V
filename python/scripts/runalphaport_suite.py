"""Run the V5 alpha-port suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("alphaenergyrunning_check", "alphaenergyrunning"),
        ("alphatorsionorder_check", "alphatorsionorder"),
        ("alphageometricport_check", "alphageometricport"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"alphaportsuite_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"alphaportsuite_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "Alphaport suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "alphaenergyrunning_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "alphatorsionorder_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "alphageometricport_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V5 alpha-port suite and summarize it.")
