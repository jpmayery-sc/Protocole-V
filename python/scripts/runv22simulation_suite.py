"""Run the V22 simulation suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v22wigner_check", "v22_wigner"),
        ("v22geometrysimulation_check", "v22_geometry_simulation"),
        ("v22stabilitysimulation_check", "v22_stability_simulation"),
        ("v22verdict_simulation_check", "v22_verdict_simulation"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v22simulation_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v22simulation_suite_summary_{timestamp}.txt"
    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
    lines = [
        "V22 simulation suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v22wigner_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v22geometrysimulation_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v22stabilitysimulation_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v22verdict_simulation_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V22 simulation suite and summarize it.")
