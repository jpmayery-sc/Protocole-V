"""Run the V22 theory suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v22geometry_check", "v22_geometry"),
        ("v22torsion_theory_check", "v22_torsion_theory"),
        ("v22hierarchy_theory_check", "v22_hierarchy_theory"),
        ("v22verdict_theory_check", "v22_verdict_theory"),
    ]
    ...
    for stem, label in report_specs:
        ...
        verdict = report.get("verdict") or report.get("theory_verdict") or "unknown"
        txt_path = report.get("txt_path") or str(report_path.with_suffix(".txt"))
        if verdict in {"supported", "supported-improved", "supported-neutral", "coherent_theory"}:
            supported += 1
        items.append(
            {
                "label": label,
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v22theory_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v22theory_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V22 theory suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v22geometry_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v22torsion_theory_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v22hierarchy_theory_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v22verdict_theory_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V22 theory suite and summarize it.")
