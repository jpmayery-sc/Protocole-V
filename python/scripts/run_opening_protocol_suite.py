"""Run the opening protocol checks and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("opening_protocol_check", "opening_protocol"),
        ("leave_one_out_collapse", "leave_one_out_collapse"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"opening_protocol_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"opening_protocol_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "Opening protocol suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    result_dir = Path(output_dir) if output_dir is not None else python_dir / "results" / "opening_protocol"

    commands = [
        [sys.executable, str(scripts_dir / "opening_protocol_check.py")],
        [sys.executable, str(scripts_dir / "leave_one_out_collapse_check.py")],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the opening protocol checks and summarize them.")
