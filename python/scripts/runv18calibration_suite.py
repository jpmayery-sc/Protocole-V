"""Run the V18 calibration suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v18delta_check", "v18_delta"),
        ("v18calibrate_check", "v18_calibrate"),
        ("v18recheck_check", "v18_recheck"),
        ("v18verdict_check", "v18_verdict"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v18calibration_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v18calibration_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V18 calibration suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v18delta_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v18calibrate_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v18recheck_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v18verdict_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V18 calibration suite and summarize it.")
