"""Run the V7 unified physics suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v7geo_check", "v7_geo"),
        ("v7electronic_check", "v7_elec"),
        ("v7coupling_check", "v7_coupling"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v7unifiedphysics_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v7unifiedphysics_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V7 unified physics suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v7geo_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v7electronic_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v7coupling_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V7 unified physics suite and summarize it.")
