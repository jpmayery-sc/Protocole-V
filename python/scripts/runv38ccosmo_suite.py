...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v38chubble_check", "v38c_hubble"),
        ("v38clambda_check", "v38c_lambda"),
        ("v38cgrowth_check", "v38c_growth"),
        ("v38ctester_summary_check", "v38c_tester_summary"),
    ]
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v38ccosmo_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v38ccosmo_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V38c cosmology suite summary",
        f"timestamp: {timestamp}",
        f"v38c_global_verdict: {summary['v38c_global_verdict']}",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v38c_cosmology"

    commands = [
        [sys.executable, str(scripts_dir / "v38chubble_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v38clambda_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v38cgrowth_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v38ctester_summary_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V38c cosmology suite and summarize it.")
