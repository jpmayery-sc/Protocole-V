"""Run the Omega(Z) structure check and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_path = latest_report(result_dir, "omega_z_structure_check")
    report = json.loads(report_path.read_text(encoding="utf-8"))
    verdict = report.get("verdict") or "unknown"

    return {
        "suite": "omega_structure",
        "timestamp": time.strftime("%Y%m%d-%H%M%SZ"),
        "overall_verdict": verdict if verdict in {"supported", "conforme strict"} else "contradicted",
        "supported_count": 1 if verdict in {"supported", "conforme strict"} else 0,
        "total": 1,
        "items": [
            {
                "label": "omega_z_structure",
                "verdict": verdict,
                "json_path": str(report_path),
                "txt_path": report.get("txt_path") or str(report_path.with_suffix(".txt")),
                "hypothesis": report.get("hypothesis") or "Omega(Z) improves the reading of heavy series by adding torsion, curvature and a normalized internal radius",
            }
        ],
    }
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"omega_structure_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"omega_structure_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "Omega structure suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    result_dir = Path(output_dir) if output_dir is not None else root / "python" / "results" / "omega_structure"

    command = [sys.executable, str(scripts_dir / "omega_z_structure_check.py")]
    run_script(command)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the Omega(Z) structure check and summarize it.")
