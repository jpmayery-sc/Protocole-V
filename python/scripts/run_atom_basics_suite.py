"""Run the atom-basics checks and summarize the result."""
...
from run_nzstability_suite import run_suite as run_nzstability_suite
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("neutral_atom_check", "neutral_atom"),
        ("iron_rust_check", "iron_rust"),
        ("h_fe_pb_check", "h_fe_pb"),
        ("nzstability_check", "nzstability"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"atom_basics_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"atom_basics_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "Atom basics suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    result_dir = Path(output_dir) if output_dir is not None else root / "python" / "results" / "atom_basics"

    commands = [
        [sys.executable, str(scripts_dir / "neutral_atom_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "iron_rust_check.py"), "--output-dir", str(result_dir)],
    ...
    run_nzstability_suite(result_dir)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the atom-basics checks and summarize them.")
