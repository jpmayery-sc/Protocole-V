"""Run the heavy border control check and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_path = latest_report(result_dir, "heavy_border_control_check")
    ...
    return {
        ...
        "items": [
            {
                "label": "heavy_border_control",
                "verdict": verdict,
                "json_path": str(report_path),
                "txt_path": report.get("txt_path") or str(report_path.with_suffix(".txt")),
                "hypothesis": report.get("hypothesis") or "Fr / Ra / Lr must remain separated at the heavy border and not collapse into one class",
            }
        ],
    }
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"heavy_border_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"heavy_border_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "Heavy border suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    result_dir = Path(output_dir) if output_dir is not None else root / "python" / "results" / "heavy_border"

    command = [sys.executable, str(scripts_dir / "heavy_border_control_check.py")]
    run_script(command)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the heavy border control check and summarize it.")
