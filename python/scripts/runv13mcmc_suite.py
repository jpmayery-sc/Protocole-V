"""Run the V13 MCMC calibration suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v13params_check", "v13_params"),
        ("v13likelihood_check", "v13_likelihood"),
        ("v13mcmc_check", "v13_mcmc"),
        ("v13postcheck_check", "v13_postcheck"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v13mcmc_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v13mcmc_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V13 MCMC suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v13params_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v13likelihood_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v13mcmc_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v13postcheck_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V13 MCMC calibration suite and summarize it.")
