...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v37bphysics_check", "v37_bphysics"),
        ("v37dark_check", "v37_dark"),
        ("v37tester_summary_check", "v37_tester_summary"),
    ]
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v37extensions_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v37extensions_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V37 extensions suite summary",
        f"timestamp: {timestamp}",
        f"v37_global_verdict: {summary['v37_global_verdict']}",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v37_extensions"

    commands = [
        [sys.executable, str(scripts_dir / "v37bphysics_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v37dark_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v37tester_summary_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V37 extensions suite and summarize it.")
