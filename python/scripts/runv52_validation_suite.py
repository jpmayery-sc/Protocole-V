"""Run the V52 validation suite and summarize it."""
...
def build_summary() -> dict[str, object]:
    ...
    return {
        "suite": "v52_validation_suite",
        "timestamp": time.strftime("%Y%m%d-%H%M%SZ"),
        "v52_global_verdict": global_verdict,
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v52_validation_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v52_validation_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V52 validation suite summary",
        f"timestamp: {timestamp}",
        f"v52_global_verdict: {summary['v52_global_verdict']}",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    root = workspace_root()
    result_dir = Path(output_dir) if output_dir is not None else root / "results" / "result-analyse" / "v52_validation"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V52 validation suite and summarize it.")
