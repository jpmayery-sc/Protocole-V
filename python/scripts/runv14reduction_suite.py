"""Run the V14 reduction suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v14sensi_check", "v14_sensi"),
        ("v14rank_check", "v14_rank"),
        ("v14reduce_check", "v14_reduce"),
        ("v14recheck_check", "v14_recheck"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v14reduction_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v14reduction_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V14 reduction suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v14sensi_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v14rank_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v14reduce_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v14recheck_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V14 reduction suite and summarize it.")
