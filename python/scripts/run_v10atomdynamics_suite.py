"""Run the V10 atomic dynamics suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v10ion_check", "v10_ion"),
        ("v10spectre_check", "v10_spectre"),
        ("v10rydberg_check", "v10_rydberg"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v10atomdynamics_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v10atomdynamics_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V10 atomic dynamics suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v10ion_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v10spectre_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v10rydberg_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V10 atomic dynamics suite and summarize it.")
