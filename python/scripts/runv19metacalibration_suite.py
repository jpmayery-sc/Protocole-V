"""Run the V19 meta-calibration suite and summarize the result."""
...
def build_summary(result_dir: Path) -> dict:
    report_specs = [
        ("v19map_check", "v19_map"),
        ("v19sensitivity_check", "v19_sensitivity"),
        ("v19robustglobal_check", "v19_robust_global"),
        ("v19verdict_check", "v19_verdict"),
    ]
...
def write_summary(summary: dict, result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v19metacalibration_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v19metacalibration_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V19 meta-calibration suite summary",
...
def run_suite(output_dir: str | Path | None = None) -> dict:
    ...
    commands = [
        [sys.executable, str(scripts_dir / "v19map_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v19sensitivity_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v19robustglobal_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v19verdict_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V19 meta-calibration suite and summarize it.")
