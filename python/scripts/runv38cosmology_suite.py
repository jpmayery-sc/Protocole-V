...
def build_summary(result_dir: Path) -> dict[str, object]:
    report_specs = [
        ("v38hubble_check", "v38_hubble"),
        ("v38lambda_check", "v38_lambda"),
        ("v38growth_check", "v38_growth"),
        ("v38tester_summary_check", "v38_tester_summary"),
    ]
...
def write_summary(summary: dict[str, object], result_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = result_dir / f"v38cosmology_suite_summary_{timestamp}.json"
    txt_path = result_dir / f"v38cosmology_suite_summary_{timestamp}.txt"

    payload = {**summary, "json_path": str(json_path), "txt_path": str(txt_path)}
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "V38 cosmology suite summary",
        f"timestamp: {timestamp}",
        f"v38_global_verdict: {summary['v38_global_verdict']}",
...
def run_suite(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    report_dir = result_dir / "v38_cosmology"

    commands = [
        [sys.executable, str(scripts_dir / "v38hubble_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v38lambda_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v38growth_check.py"), "--output-dir", str(result_dir)],
        [sys.executable, str(scripts_dir / "v38tester_summary_check.py"), "--output-dir", str(result_dir)],
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the V38 cosmology suite and summarize it.")
